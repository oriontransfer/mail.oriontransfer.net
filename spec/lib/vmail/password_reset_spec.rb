...
RSpec.describe VMail::PasswordReset do
	...
	it "should be able to reset password" do
		...
		get Trenni::URI("/reset/index", id: password_reset.id, token: password_reset.token)
		
		expect(last_response.status).to be == 200
		
		post Trenni::URI("/reset/index", id: password_reset.id, token: password_reset.token), {
	...
	it "should be fail to reset password if token is invalid" do
		get Trenni::URI("/reset/index", id: password_reset.id, token: password_reset.token + 'bad')
