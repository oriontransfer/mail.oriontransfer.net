...
on 'login' do |request|
	...
	if request.post?
		...
		unless email and password
			Console.warn(self, "Failed login attempt for email: #{email}", reason: "email and password are required")
			fail!(:unprocessible, "email and password are required")
		end
		...
		VMail.schema do |schema|
			...
			unless account&.is_admin
				Console.warn(self, "Failed login attempt for email: #{email}", reason: "account does not have admin access")
				fail!(:unauthorized, "account does not have admin access")
			end
			...
			if account&.plaintext_authenticate(password)
				...
				if account.totp_setup_required?
					...
					if totp_token && !totp_token.empty?
						...
						if account.verify_totp(totp_token)
							...
							Console.warn(self, "Failed login attempt for email: #{email}", reason: "invalid TOTP token")
							sleep 1
				...
				Console.warn(self, "Failed login attempt for email: #{email}", reason: "invalid email or password")
				sleep 1
...
on 'totp-setup' do |request|
	...
	VMail.schema do |schema|
		...
		if request.post?
			...
			unless @account.totp_secret
				Console.info(self, "TOTP setup initiated for account #{@account.email_address}")
